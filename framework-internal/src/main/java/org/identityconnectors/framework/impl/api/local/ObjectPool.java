...
public class ObjectPool<T> {
    ...
    /**
     * Map from the object to the PooledObject (use IdentityHashMap so it's always object equality)
     */
    private final Map<T, PooledObject<T>> _activeObjects = new IdentityHashMap<T, PooledObject<T>>();

    /**
     * Queue of idle objects. The one that has been idle for the longest comes first in the queue
     */
    private final LinkedList<PooledObject<T>> _idleObjects = new LinkedList<PooledObject<T>>();
    ...
    public ObjectPool(final ObjectPoolHandler<T> handler, final ObjectPoolConfiguration config) {
        ...
        _handler = handler;
        // clone it
        _config = (ObjectPoolConfiguration) SerializerUtil.cloneObject(config);
        // validate it
        _config.validate();
    ...
    public void returnObject(final T object) {
        ...
        synchronized (LOCK) {
            ...
            final PooledObject<T> pooled = _activeObjects.remove(object);

            //they are attempting to return something we haven't allocated (or that they've already returned)
            if (pooled == null) {
                throw new IllegalStateException("Attempt to return an object not in the pool: " + object);
            }

            //set it to idle and add to idle list (this might get evicted right away by evictIdleObjects if we're over 
            // the limit or if we're shutdown)
    ...
    public T borrowObject() {
        while (true) {
            final PooledObject<T> rv = borrowObjectNoTest();
            try {
                //make sure we are testing it outside of synchronization. otherwise this can create an IO bottleneck
    ...
    /**
     * Borrow an object from the pool, but don't test it (it gets tested by the caller *outside* of synchronization)
     *
     * @return the object
     */
    private PooledObject<T> borrowObjectNoTest() {
        ...
        synchronized (LOCK) {
            ...
            while (true) {
                ...
                } //otherwise, allocate a new object if below the limit
                else if (_activeObjects.size() < _config.getMaxObjects()) {
                    pooledConn = new PooledObject<T>(_handler.newObject());
                }

                //if there's an object available, return it and break out of the loop
                if (pooledConn != null) {
                    pooledConn.setActive(true);
                    _activeObjects.put(pooledConn.getObject(), pooledConn);
                    return pooledConn;
                }
    ...
    /**
     * Closes any idle objects in the pool. Existing active objects will remain alive and be allowed to shutdown
    ...
    public Statistics getStatistics() {
        synchronized (LOCK) {
            return new Statistics(_idleObjects.size(), _activeObjects.size());
        }
    }

    /**
     * Evicts idle objects as needed (evicts all idle objects if we're shutdown)
     */
    ...
    /**
     * Returns true if any of the following are true: <ol> <li>We're shutdown and there are idle objects</li> <li>Max
     * idle objects exceeded</li> <li>Min idle objects exceeded and there are old objects</li> </ol>
     */
    private boolean tooManyIdleObjects() {
        ...
        PooledObject<T> oldest = _idleObjects.getFirst();
