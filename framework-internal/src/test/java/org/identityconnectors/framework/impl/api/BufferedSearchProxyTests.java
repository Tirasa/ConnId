...
public class BufferedSearchProxyTests {
    ...
    private static class ExpectedTestResults implements ResultsHandler {
        ...
        private List<ResultsHandler> _resultsHandlers = new ArrayList<ResultsHandler>();
    ...
    private static class CheckCountHandler implements ResultsHandler {
        ...
        public boolean handle(ConnectorObject object) {
            Assert.assertEquals(_expectedCount, object.getAttributeByName(
                    "count").getValue().get(0));
            return true;
        }
    }
    ...
    public void testCancel() {
        ...
        expected.addExpectedResult(new StopResultsHandler(new CheckCountHandler(
                5)));
    ...
    public void passException() {
        ExpectedTestResults expected = new ExpectedTestResults();
        expected.addExpectedRange(0, 5);
        SearchApiOp search = new Searches.ThrowsExceptionSearch(10, 5,
    ...
    private static SearchApiOp createSearchProxy(SearchApiOp search, int bufSize, long timeout) {
        BufferedResultsProxy timeoutHandler = new BufferedResultsProxy(search,
                bufSize, timeout);
        return (SearchApiOp) Proxy.newProxyInstance(SearchApiOp.class.
                getClassLoader(),
