...
/**
 * Thread local variable that impacts localization of all messages in the connector framework. This is roughly
 * equivalent to .Net's Thread.CurrentCulture. Note that this is an inheritable thread local so it is automatically
 * inherited from parent to child thread. Of course, if the child thread is part of a thread pool, you will still need
 * to manually propagate from parent to child.
 */
public final class CurrentLocale {
     ...
     *
     * @return true iff a thread-local locale is specified on the current thread.
     */
