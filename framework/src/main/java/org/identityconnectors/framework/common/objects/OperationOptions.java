...
public final class OperationOptions {
     ...
     * specified container} to search. Must be one of the following values: <ol> <li>{@link #SCOPE_OBJECT}</li>
     * <li>{@link #SCOPE_ONE_LEVEL}</li> <li>{@link #SCOPE_SUBTREE}</li> </ol>
     */
    public static final String OP_SCOPE = "SCOPE";

    /**
     * A value of {@link #OP_SCOPE} that indicates to search for the {@linkplain #OP_CONTAINER
     * specified container} <em>object itself</em>.
     */
    public static final String SCOPE_OBJECT = "object";

    /**
     * A value of {@link #OP_SCOPE} that indicates to search for objects that the {@linkplain #OP_CONTAINER
    ...
    /**
     * An option to use with {@link SearchApiOp} that specifies the container under which to perform the search. Must be
     * of type {@link QualifiedUid}. Should be implemented for those object classes whose
     * {@link ObjectClassInfo#isContainer()} returns true.
     */
    public static final String OP_CONTAINER = "CONTAINER";

    /**
     * An option to use with {@link ScriptOnResourceApiOp} and possibly others that specifies an account under which to
     * execute the script/operation. The specified account will appear to have performed any action that the
     * script/operation performs. <p> Check the javadoc for a particular connector to see whether that connector
     * supports this option.
     */
    public static final String OP_RUN_AS_USER = "RUN_AS_USER";

    /**
     * An option to use with {@link ScriptOnResourceApiOp} and possibly others that specifies a password under which to
     * execute the script/operation.
     */
    public static final String OP_RUN_WITH_PASSWORD = "RUN_WITH_PASSWORD";

    /**
     * Determines which attributes to retrieve during {@link SearchApiOp} and {@link SyncApiOp}. <p> This option
     * overrides the default behavior, which is for the connector to return exactly the set of attributes that are
     * identified as {@link AttributeInfo#isReturnedByDefault() returned by default} in the schema for that connector.
     * <p> This option allows a client application to request <i>additional attributes</i> that would not otherwise not
     * be returned (generally because such attributes are more expensive for a connector to fetch and to format) and/or
     * to request only a <i>subset of the attributes</i> that would normally be returned.
     */
    ...
    /**
     * Returns a map of options. Each value in the map must be of a type that the framework can serialize. See
