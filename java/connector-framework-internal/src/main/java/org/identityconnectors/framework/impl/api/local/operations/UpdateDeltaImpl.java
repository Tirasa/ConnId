 ...
 * update.
 */
public class UpdateDeltaImpl extends ConnectorAPIOperationRunner implements
	...
	public Set<AttributeDelta> updateDelta(ObjectClass objclass, Uid uid, Set<AttributeDelta> modifications,
		...
		if (conector instanceof UpdateDeltaOp) {
			...
			if(!uid.getValue().get(0).equals(newUid.getValue().get(0))){
				sideEffectAttributesDelta.add(AttributeDeltaBuilder.build(Uid.NAME, newUid.getValue()));
			}
