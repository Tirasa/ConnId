...
public class UpdateDeltaImpl extends ConnectorAPIOperationRunner implements UpdateDeltaApiOp {
    ...
    public Set<AttributeDelta> updateDelta(ObjectClass objclass, Uid uid, Set<AttributeDelta> modifications,
        ...
        for (AttributeDelta attrDelta : modifications) {
            ...
            if (attrDelta.getValuesToAdd() == null
                    && attrDelta.getValuesToRemove() == null
                ...
                if (OperationalAttributes.PASSWORD_NAME.equals(name)) {
                    ...
                    if (attrDelta.getValuesToAdd() == null) {
                        throw new IllegalArgumentException(
                                "Missing values to add in non-replace password operation");
                    }
                    if (attrDelta.getValuesToRemove() == null) {
                        throw new IllegalArgumentException(
                                "Missing values to remove in non-replace password operation");
                    }
        ...
        switch (connector) {
            ...
            case UpdateAttributeValuesOp valueOp -> {
                ...
                for (AttributeDelta attrDelta : modifications) {
                    if (attrDelta.getValuesToReplace() != null) {
                        ...
                        if (attrDelta.getValuesToRemove() != null) {
                            valuesToRemove.add(AttributeBuilder.build(
                                    attrDelta.getName(), attrDelta.getValuesToRemove()));
                        }
                    }
                }
                ...
                if (!valuesToReplace.isEmpty()) {
                    try {
                        ...
                        newUid = valueOp.update(objclass, uid, valuesToReplace, options);
                    } catch (RuntimeException e) {
            ...
            default -> {
                ...
                UpdateOp op = (UpdateOp) connector;
    ...
    private Uid executeUpdateAttributeValues(
            final UpdateAttributeValuesOp valueOp,
            final String method,
            final ObjectClass objclass,
            final Uid uid,
            final Set<Attribute> valuesToUpdate,
    ...
    private static Set<AttributeDelta> normalizeSetAttributesDelta(
            final ObjectNormalizerFacade normalizer,
            final Set<AttributeDelta> attrsDelta) {
    ...
    private static List<Object> normalizeListAttributesValues(
            final ObjectNormalizerFacade normalizer,
            final String name,
