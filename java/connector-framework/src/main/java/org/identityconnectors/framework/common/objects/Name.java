 ...
 * <code>Account</code> will most often be its loginName. The value of
 * <code>Name</code> need not be unique within <code>ObjectClass</code>. In
 * LDAP, for example, the <code>Name</code> could be the
 * <code>Common Name (CN)</code>. Contrast this with {@link Uid}, which is
 ...
 * <code>Name</code> attribute to supply the user-friendly identifier for the
 * object. (Because the create operation returns the <code>Uid</code> as its
 * result, the application cannot know the <code>Uid</code> value beforehand.)</li>
 * <li>When an application renames an object, this changes the <code>Name</code>
 * of the object. (For some target resources that do not have a separate
 * internal identifier, this might also change the <code>Uid</code>. However,
 * the application would never attempt to change the <code>Uid</code> directly.)
 * </li>
 * </ul>
 * <b>NOTE:</b> For some connectors, <code>Name</code> and <code>Uid</code> will
 ...
 * <code>Uid</code> that has the same string value as the <code>Name</code>
