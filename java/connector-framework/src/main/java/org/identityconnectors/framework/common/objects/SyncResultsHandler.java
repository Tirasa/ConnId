...
/**
 * Callback interface that an application implements in order to handle results from {@link SyncApiOp} in a
...
public interface SyncResultsHandler {
     ...
     * guarantees that once an application's call to {@link SyncApiOp#sync SyncApiOp#sync()} returns, the framework will
     * no longer call this method to handle results from that {@code sync()} operation.
     *
