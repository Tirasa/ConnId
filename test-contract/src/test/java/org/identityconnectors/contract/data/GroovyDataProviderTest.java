...
public class GroovyDataProviderTest {
    ...
    public void testProperDefaulting() {
        ...
        assertTrue(message, nonExistingAttribute
        ...
        assertTrue(message, anotherNonExistingAttribute
                .equals(anotherNonExistingAttribute2));
    }

    /**
     * helper method of {@link GroovyDataProviderTest#testProperDefaulting()}; in case of missing value should return
     * default one.
     */
    ...
    public void testDotNameString() throws Exception {
        ...
        assertNotNull(o);
        assertTrue(o instanceof String);
        assertEquals("the spanish inquisition", o.toString());
    }
        ...
        if (o instanceof List<?>) {
            ...
            for (Object object : l) {
                ...
                String msg = String.format(
                        "default value for Random multiString should contain Strings only, but found different type: ",
                        object.getClass().getName());
                assertTrue(msg, object instanceof String);

                if (previous != null) {
                    msg = String.format(
                            "two objects on the randomized list should differ, but there are two equal values: %s = %s",
                            previous, object);
                    assertTrue(msg, !previous.equals(object));
                }
                previous = object;
            }
        }
    ...
    @Test
    /**
     * test {@link GroovyDataProvider#get(String)} test {@link GroovyDataProvider#get(String, int)}
     */
    ...
    @Test
    /**
     * Test queries for non-existing parameter "foo.bar" and expects a List to return based on one defined in
     * multi.Tstring. (resides in bootsrap.groovy)
     */
    @SuppressWarnings("unchecked") // collection retyping
    ...
    /**
     * helper method that retrieves the parameter from the properties file and tries to display useful information about
     * its contents on System.out
     */
    private Object getProperty(GroovyDataProvider gdp2, String propertyName,
    ...
    public void testCombinedLazyValue() {
        ...
        String msg = String.format(
                ...
                lastName, fullName);
        assertTrue(msg, String.format("%s %s", firstName, lastName).equals(fullName));
    }
    ...
    public void testGetAttributeSet() throws Exception {
        ...
        for (Attribute attr : as) {
            if (attr.getName().equals("name")) {
                assertEquals("String", attr.getValue().get(0));
            } else if (attr.getName().equals("id")) {
                assertEquals(15, attr.getValue().get(0));

            } else if (attr.getName().equals("arl")) {
                assertEquals(2, attr.getValue().size());
                assertEquals("elm1", attr.getValue().get(0));
                assertEquals("elm2", attr.getValue().get(1));
            } else if (attr.getName().equals("ara")) {
                assertEquals(2, attr.getValue().size());
                assertEquals("elm1", attr.getValue().get(0));
                assertEquals("elm2", attr.getValue().get(1));
            } else if (attr.getName().equals("bool")) {
                assertEquals(true, attr.getValue().get(0));
            } else {
    ...
    public void testGuardedStringUniqueness() {
        ...
        assertEquals(defaultedValue1, ((DataProvider) gdp).get(GuardedString.class, "nonexistingAttributeFooBarBaz_123",
                ""));
        assertEquals(defaultedValue2, ((DataProvider) gdp).get(GuardedString.class, "nonexistingAttributeFooBarBaz_456",
                ""));
    ...
    /**
     * method controls, if single parameters are correctly quoted, and multi params.
     *
